...
function candidateCost(
	...
	const anchorNative = validDistance(anchor.shapeDistTraveled);
...
function makeCandidates(
	...
	for (const projection of projections) {
		...
		candidates.push({
			...
			nativeShapeDistance: projection.nativeShapeDistance ?? null,
			coordinateSource: projection.coordinateSource,
			cost: candidateCost(
				projection.lateralDistanceMeters,
				projection.nativeShapeDistance ?? null,
				anchor,
				shape,
			),
		});
	...
	if (anchorNative != null) {
		...
		if (native) {
			...
			candidates.push({
				...
				cost: candidateCost(projection?.lateralDistanceMeters ?? 0, native.nativeShapeDistance, anchor, shape),
			});
		}
	}
...
function solveOrientation(
	...
	for (let index = 0; index < anchors.length; index++) {
		const candidates = makeCandidates(anchors[index], index, shape, config, anchors.length, orientation);
...
export function alignShapeAnchors(
	...
	const forward = toAlignment(solveOrientation(anchors, shape, config, "forward"), anchors, shape, "forward");
	const reverse = toAlignment(solveOrientation(anchors, shape, config, "reverse"), anchors, shape, "reverse");
	...
	if (hasComparableAlternative(anchors, shape, config, selected.orientation, selected)) selected.ambiguous = true;
	return selected;
}
