...
export async function refreshStaticCache(skipRealtimeOverlap: boolean = false): Promise<void> {
    ...
    if (skipRealtimeOverlap) logger.debug("Skipping augmenting trips.", { module: "cache", function: "refreshStaticCache" });
    else {
    ...
    if (skipRealtimeOverlap) logger.debug("Skipping building augmented stop times cache.", { module: "cache", function: "refreshStaticCache" });
    else for (const trip of augmentedCache.trips) {
