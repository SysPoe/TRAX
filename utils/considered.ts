...
export function isConsideredStop(stop: AugmentedStop | qdf.Stop, gtfs?: qdf.GTFS): boolean {
	...
	const valid = gtfs.queryStopTimes({
		stop_id: stop.stop_id
	}).some(st => isConsideredTripId(st.trip_id, gtfs)) || children.some(child => isConsideredStopId(child, gtfs));
...
export function isConsideredStopId(stop_id: string, gtfs?: qdf.GTFS): boolean {
	...
	const stop = (gtfs ?? getGtfs()).getStop(stop_id);
