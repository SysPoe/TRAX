...
export function findExpressString(tripStops: gtfs.Stop[], stop_id: string): string {
	...
	return segments
		.map((run) => {
			...
			return run.from == cache.getRawStops(stop_id)[0]?.parent_station || run.from == stop_id
