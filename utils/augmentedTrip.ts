...
export function toSerializableAugmentedTrip(
	trip:
		...
		| Omit<
				AugmentedTrip,
				"toSerializable" | "toSerializable" | "_runSeries"
		  >,
	...
	return {
		...
		get scheduledTripDates() {
			...
			if (stopTimes.length > 0) stopTimesToUse = stopTimes;
				...
				if (!cachedStopTimes)
					cachedStopTimes = augmentStopTimes(
						rawStopTimes,
						serviceDates,
					);
	...
	for (let _break = 0; _break < 100; _break++) {
		...
		if (
			(st.scheduled_departure_timestamp ||
				st.scheduled_arrival_timestamp) === null
		)
			break;
		...
		deps = deps.filter(
			(v) =>
				v._stopTime?.trip_id.slice(-4)[0] == run[0] &&
				v._stopTime?.trip_id.slice(-4) != run,
		);

		deps = deps.sort(
			(a, b) =>
				(a.scheduled_departure_timestamp ||
					a.scheduled_arrival_timestamp ||
					Infinity) -
				(b.scheduled_departure_timestamp ||
					b.scheduled_arrival_timestamp ||
					Infinity),
		);
		...
		if (!newTrip) break;
		if (
			deps.at(-1)?._stopTime?.stop_sequence !=
			newTrip.stopTimes.at(-1)?._stopTime?.stop_sequence
		)
			break;
	...
	for (let _break = 0; _break < 100; _break++) {
		...
		if (!rs.trips.some((v) => v.trip_id === newTrip._trip.trip_id))
			rs.trips.push({
				...
				trip_start_time:
					newTrip.stopTimes[0].scheduled_departure_timestamp ||
