...
export function augmentStopTimes(
    ...
    for (let passingStopTime of passingStopTimes) {
        ...
        let realtimeUpdate = realtimeUpdates.find(
            (update) => update.stop_id == stopId || actualParentStation?.stop_id == update.stop_id || cache.getAugmentedStops(stopId)[0].children.some((child) => child.stop_id == update.stop_id)
        );
