...
export function augmentStopTimes(stopTimes: qdf.StopTime[], serviceDates: string[], explicitTripId?: string): AugmentedStopTime[] {
    ...
    const staticMap = new Map<number, PassingStopTime>();
    ...
    const rtMap = new Map<number, qdf.RealtimeStopTimeUpdate>();
    ...
    const allSequences = new Set<number>([...staticMap.keys(), ...rtMap.keys()]);
        ...
        if (rtUpdate) {
            ...
            if (rtUpdate.departure_delay !== null && rtUpdate.departure_delay !== undefined) {
                ...
                rtFlags.dep = true;
                 ...
                 if (time) {
                     actDep = time;
                     if (rtUpdate.departure_delay !== null && rtUpdate.departure_delay !== undefined) delaySecs = rtUpdate.departure_delay;
                     else delaySecs = 0;
                     lastDelay = delaySecs;
                     rtFlags.dep = true;
                 }
            ...
            if (rtUpdate.arrival_delay !== null && rtUpdate.arrival_delay !== undefined) {
                ...
                rtFlags.arr = true;
                 ...
                 if (time) {
                     actArr = time;
                     if (rtUpdate.arrival_delay !== null && rtUpdate.arrival_delay !== undefined) delaySecs = rtUpdate.arrival_delay;
                     else delaySecs = 0;
                     lastDelay = delaySecs;
                     rtFlags.arr = true;
                 }
            ...
            // No direct update
            if (lastDelay !== 0) {
                if (schedArr !== null && schedArr !== undefined) actArr = schedArr + lastDelay;
                if (schedDep !== null && schedDep !== undefined) actDep = schedDep + lastDelay;
                propagated = true;
