...
function convertQRTServiceToTravelTrip(service, serviceResponse, direction, line) {
    ...
    const stops = serviceResponse.TrainMovements.map((movement) => {
        ...
        if (movement.PlaceName.toLowerCase().includes("roma st"))
            gtfsStopId = "place_romsta";
        let toRet = {
        ...
        if (serviceResponse.Success) {
            ...
            if (qrtService) {
                ...
                const trainMovements = travelTrip.stops.map((s) => {
                    ...
                    if (findRes)
                        gtfsStopId = findRes.stop_id;
                    if (s.placeName.toLowerCase().includes("roma st"))
                        gtfsStopId = "place_romsta";
                    return {
...
export async function getCurrentQRTravelTrains(retries = 5) {
    try {
