...
function getSRTMinutes(from, to) {
    const match = SRT.find((row) => row.from.toLowerCase() === from.toLowerCase() &&
...
export function expandWithSRTPassingStops(stops) {
    if (stops.length === 0)
        return [];
    // Build a list of all PlaceNames in the SRT route (SEQ only)
    const srtStations = Array.from(new Set(SRT.flatMap((row) => [row.from, row.to])));
    // Find the first and last stop in the input that is in the SRT region
    let firstIdx = stops.findIndex((s) => srtStations.some((name) => name.toLowerCase() === s.PlaceName.toLowerCase()));
    let lastIdx = (() => {
    ...
    const stopIdxByName = new Map(stops.map((s, i) => [s.PlaceName.toLowerCase(), i]));
    ...
    for (let i = firstIdx + 1; i <= lastIdx; ++i) {
        ...
        let between = [];
        let pIdx = srtStations.findIndex((name) => name.toLowerCase() === prev.PlaceName.toLowerCase());
        let cIdx = srtStations.findIndex((name) => name.toLowerCase() === curr.PlaceName.toLowerCase());
        ...
        if (srtMins == null)
            srtMins = 0;
        if (prevTime)
            prevTime = new Date(prevTime.getTime() + srtMins * 60000);
        result.push({
            ...curr,
            passing: false,
