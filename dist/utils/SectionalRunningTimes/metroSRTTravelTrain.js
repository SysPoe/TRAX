...
export function expandWithSRTPassingStops(stoppingMovements) {
    ...
    for (let i = 0; i < stoppingMovements.length - 1; ++i) {
        ...
        let queue = SRT_DATA.filter(s => s.from === from.PlaceName).map(s => ({ path: [s], last: s.to }));
        let found = null;
        let visited = new Set();
        ...
        if (found) {
            ...
            for (let j = 1; j < found.length - 1; ++j) {
                ...
                if (foundSeg) {
                    ...
                    let estPass = prevTime && foundSeg.travelTrain ? new Date(prevTime.getTime() + foundSeg.travelTrain * 60000) : undefined;
                    result.push({
            ...
            let estArr = prevTime && lastSeg && lastSeg.travelTrain ? new Date(prevTime.getTime() + lastSeg.travelTrain * 60000) : undefined;
            result.push({
                placeName: to.PlaceName,
                isStop: true,
                plannedArrival: to.PlannedArrival,
